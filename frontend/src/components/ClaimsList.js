...
Generated by BS Detector - https://bs-detector-transcriber.vercel.app
  ...
  const handleRecheck = async (e) => {
    ...
    try {
      ...
      alert(`Failed to re-check claim: ${error.response?.data?.error || error.message}`);
    } finally {
    ...
    <div className={`claim-item ${config.color} ${expanded ? 'expanded' : ''} ${errorReported ? 'error-reported' : ''}`}>
      ...
      {expanded && (
        <div className="claim-details">
          {/* Action buttons - shown in expanded state */}
          <div className="claim-details-actions">
            {videoId && (
              ...
              >
                {recheckLoading ? '⏳ Re-checking...' : '🔄 Re-check Claim'}
              </button>
            )}
            <button 
            ...
            >
              {errorReported ? '✓ Reported' : '⚠️ Report Error'}
            </button>
          </div>
