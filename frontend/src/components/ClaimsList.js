...
💡 CONFIDENCE: ${claim.confidence || 'Not specified'}
...
Generated by BS Detector - https://bs-detector-transcriber.vercel.app
  ...
  const handleRecheck = async (e) => {
    ...
    if (!videoId) {
      console.error('❌ Cannot re-check: videoId is missing!', { videoId });
    ...
    try {
      console.log('🔍 Re-checking claim...', { videoId, claim: claim.claim.substring(0, 50) + '...' });
      ...
      console.log('✅ Re-check result:', response.data);
      ...
      if (errorMessage.includes('model') || errorMessage.includes('404')) {
        errorMessage += '\n\n🔧 The AI model may need updating. Please contact support or try again later.';
      }
    ...
    <div className={`claim-item ${config.color} ${expanded ? 'expanded' : ''} ${errorReported ? 'error-reported' : ''}`}>
      <div className="claim-header" onClick={() => setExpanded(!expanded)}>
        ...
        <div className="claim-actions">
          ...
          {videoId ? (
            ...
            <button 
              className="claim-recheck"
              disabled={true}
              title="Re-check not available (video not saved)"
      ...
      {expanded && (
        <div className="claim-details">
          ...
          {claim.logical_fallacies && claim.logical_fallacies.length > 0 && (
            <div className="claim-fallacies">
              <h4>🧠 Logical Analysis:</h4>
              <ul>
                {claim.logical_fallacies.map((fallacy, idx) => (
                  <li key={idx} className={fallacy.toLowerCase() === 'none detected' || fallacy.toLowerCase() === 'sound reasoning' ? 'fallacy-none' : 'fallacy-detected'}>
                    {fallacy.toLowerCase() === 'none detected' || fallacy.toLowerCase() === 'sound reasoning' ? (
                      <span>✅ {fallacy}</span>
                    ) : (
                      <span>⚠️ {fallacy}</span>
                    )}
          ...
          {recheckResult && (() => {
            ...
            const getConfidenceIcon = (original, updated) => {
              ...
              if (updLevel > origLevel) return ' ⬆️';
              if (updLevel < origLevel) return ' ⬇️';
              return ' ✓';
            };
            ...
            return (
              <div className="recheck-result">
                <div className="recheck-header">
                  <strong>🔍 Deep Re-check Results:</strong>
                  {recheckResult.changed && (
                    <span className="verdict-changed">⚠️ Verdict Changed!</span>
                  )}
                </div>
...
function ClaimsList({ data, videoId }) {
  ...
  // Debug logging
  console.log('🔍 ClaimsList - videoId:', videoId);
  console.log('🔍 ClaimsList - videoId type:', typeof videoId);
  console.log('🔍 ClaimsList - Should show recheck button?', !!videoId);
  console.log('🔍 ClaimsList - data keys:', data ? Object.keys(data) : 'null');
  console.log('🔍 ClaimsList - data:', data);
  
  if (!data) return null;
  
  const verifiedClaims = data.verified_claims || [];
  // Handle both "opinion_claims" and "opinion_based_claims" (Claude sometimes uses different key)
  const opinionClaims = data.opinion_claims || data.opinion_based_claims || [];
